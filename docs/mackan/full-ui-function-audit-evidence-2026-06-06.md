...
| MAINT-REPAIR-001 | P1 | Repair Registry failed on the selected real instance with duplicate key `GameData/ContractPacks`; MACKAN surfaced a typed failed result, but the repair did not complete. | `MACKAN.Service/CoreMackanMaintenanceProvider.cs`; CKAN Core registry repair/unmanaged scan path | open | Failing artifact: `task14/maintenance-service-transcript.json`; summary: `task14/maintenance-service-summary.json`; tests still passed in `task14-dotnet-maintenance-net10.log`, so this is live-instance data dependent rather than covered by the current fixture set. |
