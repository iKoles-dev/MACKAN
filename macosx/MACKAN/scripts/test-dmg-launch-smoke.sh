...
usage() {
    cat <<USAGE
Usage: test-dmg-launch-smoke.sh [--timeout SECONDS] [--app-name NAME] [--require-version VERSION] DMG_PATH

Mounts a MACKAN DMG, verifies a bundled app is present, and runs launch smoke
checks through verify-app-launch.sh.
USAGE
}

cleanup() {
    if [[ -n "$MOUNT_DIR" && -d "$MOUNT_DIR" && "$MOUNTED" == true ]]; then
        for _ in {1..12}; do
...
action_tmp="${TMPDIR:-/tmp}"
TMP_WORK_DIR="$(mktemp -d "${action_tmp}/mackan-dmg-smoke.XXXXXX")"
...
if [[ -f "$INFO_PLIST" ]]; then
    ...
    if [[ -n "$REQUIRED_VERSION" ]]; then
        [[ -n "$BUNDLE_SHORT_VERSION" ]] || {
            echo "CFBundleShortVersionString is missing in $INFO_PLIST" >&2
            exit 1
        }
        [[ -n "$BUNDLE_VERSION" ]] || {
            echo "CFBundleVersion is missing in $INFO_PLIST" >&2
            exit 1
        }
...
"$VERIFY_LAUNCH" --timeout "$TIMEOUT_SECONDS" "$APP_PATH"

echo "Verified DMG launch smoke: $DMG_PATH"
